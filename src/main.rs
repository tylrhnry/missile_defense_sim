...
fn main() {
    ...
    let enemy_heading = Heading {
        x: -1.0,
        y: 0.0,
        z: 0.0,
    }; // facing straight down the y axis
    let enemy_vel = Velocity {
        speed: 360.0,
        heading: enemy_heading,
    }; // 950 km/h
    let enemy_target_pos = Position {
        x: 0.0,
        y: 100_000.0,
        z: 250_000.0,
    };
    let enemy_detonation_dist = 3.0;
    let mut enemy = EnemyMissile::new(
        enemy_pos,
        enemy_vel,
        enemy_target_pos,
        enemy_detonation_dist,
        None,
    );
    ...
    let interceptor_pos = Position {
        x: 0.0,
        y: 0.0,
        z: 200_000.0,
    };
    let interceptor_heading = Heading {
        x: 0.0,
        y: 1.0,
        z: 0.0,
    }; // facing straight up
    let interceptor_vel = Velocity {
        speed: 500.0,
        heading: interceptor_heading,
    }; // 4285 km/h
    let interceptor_target_pos = enemy.pos.clone();
    let interceptor_detonation_dist = 3.0;
    let interceptor_max_accel = MissileAccel {
        _forward: 30.0,
        _backward: 20.0,
        _roll: 20.0,
        _attitude: 40.0,
    ...
    let mut interceptor = InterceptorMissile::new(
        interceptor_pos,
        interceptor_vel,
        interceptor_detonation_dist,
        interceptor_target_pos,
        interceptor_max_accel,
        interceptor_flight_range,
    );
    ...
    let root = BitMapBackend::gif("missile_paths.gif", (1920, 1080), 150)
        .unwrap()
        .into_drawing_area();
    ...
    while let (MissileState::Normal, MissileState::Normal) = (
        ...
        if (iter_count % 5_000 == 0)
            | ((abs_dist(&interceptor.pos, &enemy.pos) < 10_000.0) & (iter_count % 1_000 == 0))
        {
            // draw frame
...
trait UpdateMissile {
    ...
    fn get_new_heading(current_pos: &Position, target_pos: &Position) -> Heading {
        ...
        let raw_vector = Heading {
            x: x_diff,
            y: y_diff,
            z: z_diff,
        }; // the vector that connects
           // the two points
        let magnitude = (raw_vector.x * raw_vector.x
            + raw_vector.y * raw_vector.y
            + raw_vector.z * raw_vector.z)
            .sqrt();
    ...
    fn update_pos(&mut self, time_step: f64) -> MissileState {
        ...
        if abs_dist(&new_pos, target_pos) < detonation_dist {
            // go define abs_dist()
